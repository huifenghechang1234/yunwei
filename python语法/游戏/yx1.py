...
2048
1. 游戏简介
2048 是一款比较流行的数字游戏。游戏规则：每次可按上、下、左、右方向键滑动数字，每滑动一次，
所有数字都会往滑动方向靠拢，同时在空白位置随机出现一个数字，相同数字在靠拢时会相加。不断叠
加最终拼出 2048 这个数字算成功。2048 最早于 2014年3月20日发行。原版 2048 首先在 GitHub 
上发布，原作者是 Gabriele Cirulli，后被移植到各个平台。本例难度为初级，适合具有 Python 
基础和 Pygame 编程知识的用户学习
"""
